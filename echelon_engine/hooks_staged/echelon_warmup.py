...
def _safe_main() -> None:
    ...
    # -- ROOM DOOR (BRIEF W3): resume the live workroom brief ABOVE the offering, bounded 3s --
    try:
        import threading as _th
        from echelon_engine.workcycle import room_path, resume_brief  # guarded: sibling module
        _room = room_path(cwd or None)
        _brief_out: list = []
        def _room_run() -> None:
            try:
                _b = resume_brief(_room) if _room else ""
                if _b:
                    _brief_out.append(_b)
            except Exception:
                pass
        _t = _th.Thread(target=_room_run, daemon=True)
        _t.start()
        _t.join(3)
        if _brief_out:
            sys.stdout.write(_brief_out[0].encode("ascii", "replace").decode("ascii") + "\n")
    except ImportError:
        sys.stdout.write("ROOM unavailable (no workcycle)\n")
    except Exception:
        pass
    ...
    if os.environ.get("ECHELON_FULL_BOOT", "").lower() not in ("1", "true", "on"):
        try:
            ...
            # P1-3 fix (OPEN-0060 phase A gate finding): the lean summoning text has no
            # .eos awareness of its own, so it must not swallow the FRAMEWORK DOOR banner
            # on the way out this early-return path — attach it independently here, same
            # as the full-boot build below.
            try:
                from echelon_eos_awareness import eos_banner
                _banner = eos_banner(cwd)
                if _banner:
                    _lean = _lean.rstrip("\n") + "\n\n" + _banner + "\n"
            except Exception:
                pass
